...
function initListarSolicitudes() {
    ...
    window.confirmReject = function() {
        ...
        console.log(`Solicitud ${currentSolicitudId} rechazada. Razón: ${reason}`);
        alert(`Solicitud ${currentSolicitudId} rechazada.${reason ? '\nRazón: ' + reason : ''}`);
        closeRejectModal();
    };

    function confirmarEliminar(solicitudId, solicitante) {
    if (confirm(`¿Estás seguro de que deseas eliminar la solicitud de ${solicitante}?`)) {
        // Hacer petición AJAX o redirect
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (confirm('¿Estás seguro de que deseas eliminar esta solicitud?')) {
                ...
                // window.location.href = this.href; // Descomenta para redireccionar
            }
        });
    });
}
...
function initSolicitudesPendientes() {
    ...
    rejectButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            openRejectModal(solicitudId, solicitanteName);
        });
    });
}
